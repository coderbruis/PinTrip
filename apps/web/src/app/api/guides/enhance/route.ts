...
export async function POST(request: Request) {
  ...
  if (!isEnhancementRequest(body)) {
    return NextResponse.json({ error: "缺少基础攻略或原始旅行需求" }, { status: 400 });
  }

  const agentUrl = getXhsAgentUrl();

  try {
    const agentResponse = await fetch(`${agentUrl}/agent/xhs-guide/enhance`, {
      method: "POST",
...
function isCompletedEnhancement(payload: unknown): payload is {
